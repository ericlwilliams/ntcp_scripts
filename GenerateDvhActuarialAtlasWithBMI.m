function GenerateDvhActuarialAtlasWithBMI(analy_str,structure_str,tox_str,data_loc,atlas_loc)
...
CGgrp_orig = [CGdata.mGrp];
clear CGdata;

% Remove patients with no BMI
bmi_data = [CGgrp_orig.mBMI];
bmi_idx = bmi_data>0;
CGgrp_orig = CGgrp_orig(bmi_idx);

% CGgrp(logical(logical([CGgrp.mBMI]>0).*logical([CGgrp.mBMI]<=30)))
% two loops, BMI <=30, BMI > 30
bmi_name = {'bmi_leq_30_','bmi_gt_30_'};
for j=1:2
    cur_bmi_name = bmi_name{j};
    obese_flg = [CGgrp_orig.mBMI]>30;
    
if j==1
    CGgrp = CGgrp_orig(~obese_flg);
elseif j==2
    CGgrp = CGgrp_orig(obese_flg);
end
...
compdate = inf(length(CGgrp),1);
lastfollowup = inf(length(CGgrp),1);
...
for i=1:length(fxs),
    ...
    if cur_fx<0
       write_atlas_file_name = [atlas_file_name,cur_bmi_name,'allfr']; 
    else
        write_atlas_file_name = [atlas_file_name,cur_bmi_name,num2str(cur_fx),'fr'];
    end
